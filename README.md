...
```bash
cd /Users/max/Developer/10_active/001_carbon-constrained-portfolio
python3 -m venv .venv
...
docs/         项目文档与重大 Change Request
notebooks/    教学解释、单期优化和滚动回测
notes/        project_log、prompt_log 和 change_log
prompts/      原始想法、Codex Brief 和 prompt 模板
...
本项目是历史项目补档，早期并未按新 Developer prompt workflow 保存原始 prompt。因此：

- `prompts/01_原始想法.md`：根据现有项目文档反推的原始想法，不是逐字历史 prompt。
- `prompts/03_Codex执行Brief.md`：根据 README、PROJECT_REPORT、CODE_NOTES 和学习笔记反推的 Codex Brief。
- `prompts/templates/`：复制自 Developer 默认项目模板，用于后续研究讨论、实现 brief、回测审查、项目复盘和 Change Request。
- `notes/prompt_log.md`：记录 prompt 资产补档和未来 prompt 迭代。

这些文件用于项目接续和复盘，不改变当前研究代码或已有输出。

## 变更管理方式

- 小的实现 bug：可直接修改代码，并记录到 `notes/change_log.md`。
- 研究逻辑、实验设计、数据口径或框架变化：先写 `docs/change_requests/CR-XXX_short-title.md`。
- 涉及新的回测结果时：不要覆盖现有 `outputs/`；优先使用 `outputs/v2/` 或新的报告文件。
- 修改完成后：同步更新 `notes/project_log.md`、`notes/change_log.md`，必要时更新 `notes/prompt_log.md` 和 README。
- 如果只是学习笔记或 networking 话术更新，不应被包装成研究结论变化。
...
3. 引入 covariance shrinkage，比较权重稳定性。
4. 测试不同 carbon budget ratio，并完整报告敏感性，而不是只展示表现最好的参数。
